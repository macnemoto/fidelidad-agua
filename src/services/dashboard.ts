...
import type { DashboardActivity, DashboardClient, DashboardSummary, DailyPurchase, ClientStatus } from '../types/dashboard'

function datesFor(period: 7 | 30 | 90 | 'all') {
  if (period === 'all') return { p_from: null, p_to: null }
  const from = new Date()
  from.setDate(from.getDate() - (period - 1))
  return { p_from: from.toISOString().slice(0, 10), p_to: new Date().toISOString().slice(0, 10) }
}
