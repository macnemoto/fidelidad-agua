...
export default function App() {
  ...
  const saveCreate = async () => { const error = validate(createName, createCedula); if (error) { setMessage({ kind: 'error', text: error }); return } setBusy(true); try { const saved = await saveClientV2('create', createCedula, createName, createCount, 0); setClient(saved); setEditName(saved.name); setEditCount(saved.purchase_count); await loadHistory(saved.cedula); setMode('search'); setSearchCedula(saved.cedula); setMessage({ kind: 'success', text: 'Cliente creado correctamente.' }) } catch (error) { setMessage({ kind: 'error', text: error instanceof Error ? error.message : 'No se pudo crear el cliente.' }) } finally { setBusy(false) } }
  const saveEdit = async () => { if (!client) return; const error = validate(editName, client.cedula); if (error) { setMessage({ kind: 'error', text: error }); return } setBusy(true); try { const saved = await saveClientV2('update', client.cedula, editName, editCount, client.purchase_count); setClient(saved); setEditName(saved.name); setEditCount(saved.purchase_count); await loadHistory(saved.cedula); setMessage({ kind: 'success', text: 'Cambios guardados.' }) } catch (error) { setMessage({ kind: 'error', text: error instanceof Error ? error.message : 'No se pudo guardar.' }) } finally { setBusy(false) } }
